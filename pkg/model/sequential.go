...
func (sequential *Sequential) Train(inputs, targets []tensor.Tensor, alpha, momentum float64, epochs, batch, verbose int, loss func(outputs, targets tensor.Tensor) (tensor.Tensor, error)) error {
	if len(inputs) != len(targets) {
		return errors.New("inputs and targets len are different")
	}
	...
	for epoch := 1; epoch <= epochs; epoch++ {
		...
		for i := 0; i < batch; i++ {
			b_loss, err = sequential.train(inputs[i], targets[i], alpha, momentum, loss)
			if err != nil {
				return err
			}
			if verbose == 2 {
				b_loss = b_loss.Abs()
				p_loss = b_loss.Sum() / float64(b_loss.Size())
				fmt.Printf("\rBatch: %d / %d [%.2f%%] => ( Loss: %f )", i, batch, float64(i+1)/float64(batch)*100.0, p_loss)
			} else if verbose == 1 {
				b_loss = b_loss.Abs()
				p_loss += b_loss.Sum() / float64(b_loss.Size())
			}
		...
		if verbose == 1 {
			p_loss /= float64(batch)
			fmt.Printf("\rEpoch: %d / %d [%.2f%%] => ( Loss: %s )", epoch, epochs, float64(epoch)/float64(epochs)*100.0, fmt.Sprint(p_loss))
		}
	}
	fmt.Println()
	return nil
...
func (sequential *Sequential) train(input, target tensor.Tensor, alpha, momentum float64, loss func(outputs, targets tensor.Tensor) (tensor.Tensor, error)) (tensor.Tensor, error) {
	sequential.OutLayer.Reset()
